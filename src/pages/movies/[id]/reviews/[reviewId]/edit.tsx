...
const Review = ({
  ...
  async function handleSubmit(
    ...
    try {
      const response = await axios.put<{ review: Serialized<Review> }>(
        `/api/reviewers/${session?.user.id as string}/reviews/${review.id}`,
        {
          title: title,
          content: content,
          rating: rating,
      ...
      await router.push(
        `/movies/${movie.id}/reviews/${response.data?.review.id}`
      );
    } catch (e) {
      setUpdateLoading(false);
      console.log(e);
...
export const getServerSideProps = async (
  ...
  if (!existingReview) {
    return {
      redirect: {
        permanent: false,
        destination: `/movies/${movieId}/reviews/create`,
      },
    };
  }
  ...
  return {
    props: {
      ...
      review: serialize(existingReview),
    },
  };
};
