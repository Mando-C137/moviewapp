...
export const generateMetadata = async ({
  ...
  const { reviewer } = await getServerData(userId);
  return {
    title: reviewer.name as string,
  };
};
...
export default async function ReviewerPage({
  ...
  const { reviewer, userIsOwner } = await getServerData(userId);
...
const getServerData = cache(async (userId: string) => {
...
type InferredReviews = Awaited<
  ReturnType<typeof getServerData>
>["reviewer"]["reviews"][number];

type InferredFavorites = Awaited<
  ReturnType<typeof getServerData>
