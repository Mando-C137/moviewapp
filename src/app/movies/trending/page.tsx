...
export default async function TrendingMoviesPage() {
  ...
  return (
    <Routepage name={"Moview - Trending"} action={{ type: "reset" }}>
...
const getServerdata = async () => {
  const fetchedMovies = await TMDB_API.fetchPopularMovies();
  const movies = await insertManyMovies(
    await filterMoviesWithUnknownImdbId(fetchedMovies),
  );
  if (movies === "error") {
    throw new Error("An error occurred when fetching the movies");
  }
  return movies;
};
